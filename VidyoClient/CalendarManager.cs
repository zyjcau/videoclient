...
namespace VidyoClient
{
	public class CalendarManager: IDisposable {
		...
		private OnGetServicesComplete _mOnGetServicesComplete;
		[DllImport(importLib, CallingConvention = CallingConvention.Cdecl)]
		...
		public Boolean GetCalendarRegisteredServicesAsync(IGetCalendarRegisteredServicesAsync _iIGetCalendarRegisteredServicesAsync){
			...
			_mOnGetServicesComplete = OnGetServicesCompleteDelegate;
...
[ObjCRuntime.MonoPInvokeCallback(typeof(OnGetServicesComplete))]
		...
		private static void OnGetServicesCompleteDelegate(IntPtr cm, IntPtr svcNames, CalendarManagerGetServicesResult result){
			CalendarManager csCm = null;
			if(cm != IntPtr.Zero){
				if(CalendarManager.VidyoCalendarManagerGetUserDataNative(cm) == IntPtr.Zero)
					csCm = new CalendarManager(cm);
				else{
					GCHandle objHandle = (GCHandle)CalendarManager.VidyoCalendarManagerGetUserDataNative(cm);
					csCm = (CalendarManager)objHandle.Target;
				}
			}
			List<String> csSvcNames = new List<String>();
			int nSvcNamesSize = 0;
			IntPtr nSvcNames = VidyoCalendarManagerOnGetServicesCompleteGetsvcNamesArrayNative(svcNames, ref nSvcNamesSize);
			int nSvcNamesIndex = 0;
			while (nSvcNamesIndex < nSvcNamesSize) {
				csSvcNames.Add((string)MarshalPtrToUtf8.GetInstance().MarshalNativeToManaged(Marshal.ReadIntPtr(nSvcNames + (nSvcNamesIndex * Marshal.SizeOf(nSvcNames)))));
				nSvcNamesIndex++;
			...
			if(csCm._mIGetCalendarRegisteredServicesAsync != null)
