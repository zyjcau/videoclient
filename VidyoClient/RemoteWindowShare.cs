...
[ObjCRuntime.MonoPInvokeCallback(typeof(OnRemoteWindowShareFrame))]
		...
		private static void OnRemoteWindowShareFrameDelegate(IntPtr w, IntPtr participant, IntPtr videoFrame){
			RemoteWindowShare csW = null;
			if(w != IntPtr.Zero){
				if(RemoteWindowShare.VidyoRemoteWindowShareGetUserDataNative(w) == IntPtr.Zero)
					csW = new RemoteWindowShare(w);
				else{
					GCHandle objHandle = (GCHandle)RemoteWindowShare.VidyoRemoteWindowShareGetUserDataNative(w);
					csW = (RemoteWindowShare)objHandle.Target;
				}
			}
			...
			if(csW._mIRegisterFrameEventListener != null)
